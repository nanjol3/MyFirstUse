...
from .modules import MaskedLinear, MaskedResidualBlock
...
class MADE(nn.Module):
    ...
    def fit(self, dataframe=None, constraints=[], verbose=True, show_progress=False,
            **kwargs):
        """Train ar synthesizer based on the train data.

        Args:
            dataframe (pd.Dataframe): The train data.

            constraints (list): The list of constraint.

            save_file (str): The path to save checkpoint. If it is ``None``, it will not save checkpoint.

            save_frequency (int): The frequency to save checkpoint. If it is ``None`` or ``0``,
                it will not save checkpoint.
        ...
        loader = DataLoader(transformed_data, batch_size=self.batch_size, shuffle=True)
        self.optimizer = torch.optim.Adam(self.parameters())
        for epoch_idx in range(self.start_epoch, self.epochs):
            start_time = time.time()
            losses = self._train_epoch(loader, constraints, epoch_idx, show_progress=show_progress, **kwargs)
            train_loss = sum(losses) if isinstance(losses, tuple) else losses
            end_time = time.time()
            if verbose:
                print("epoch %d: train loss %.3f, time cost %.3fs" % (epoch_idx, train_loss, end_time - start_time))

    def _train_epoch(self, loader=None, constraints=[], epoch_idx=None, loss_func=None, show_progress=False, **kwargs):
        """Train ar synthesizer in an epoch.

        Args:
            loader (torch.utils.Dataloader): The train data.

            constraints (list): The list of constraint.

            epoch_idx (int): The current epoch index.

            loss_func (function): The loss function. If it is ``None``, the loss function will be

                `self.synthesizer.calculate_loss`. Defaults to ``None``.
        ...
        self.train()
        total_loss = 0.
        loss_func = loss_func or self.calculate_loss
        iter_data = (
            tqdm(loader)
            if show_progress
            else loader
        ...
        for batch_idx, batch_data in enumerate(iter_data):
            ...
            loss.backward()
            nn.utils.clip_grad_norm_(self.parameters(), max_norm=10, norm_type=2)
            self.optimizer.step()
        return total_loss / len(iter_data)

    @staticmethod
    def _check_nan(loss):
        if loss is None or loss == 0. or torch.isnan(loss):
            return True
        else:
            return False
    ...
    def sample(self, sample_num=1, conditions=None, device=None, inverse=True):
        """MADE sample.
        ...
        with torch.no_grad():
            ...
            for i in range(self.nin):
                ...
                sampled[:, i] = s.view(-1,)
        if inverse:
            sampled = self.table.inverse(sampled.cpu().numpy())
            return sampled
        else:
            return sampled.cpu()
